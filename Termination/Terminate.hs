...
instance Pretty TagBag where
    pPrint (TagBag m) = braces $ hsep $ punctuate (text ",")
      [pPrint n | (n, count) <- IM.toList m, _ <- replicate count n]
...
terminate hist here
  ...
  | tERMINATION_CHECK && any (<| here) hist = Stop
  | otherwise = Continue (here : hist)
